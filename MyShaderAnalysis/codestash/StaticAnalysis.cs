    ...
    public class StaticAnalysis1
    {
        ...
        static void DBlockRuleKeyValuesAnalysis()
        {
            ...
            foreach (string vcsFilenamepath in allVcsFiles)
            {
                ...
                foreach (ConstraintsBlock uknBlock in shaderFile.DConstraintsBlocks)
                {
                    ...
                    if (uknBlock.Range2[0] == 0)
                    {
                        Console.WriteLine($"{ShortHandName(vcsFilenamepath)[1..],-55} {GetConciseDescription(uknBlock, new int[] { 8, 4, 7, 5 })}" +
                            $"{shaderFile.DBlocks[0].Name,-33}");
                    }

                    if (uknBlock.Rule == ConditionalRule.Requires && AllFlagsAre3(uknBlock))
                    {
                        Console.WriteLine($"{ShortHandName(vcsFilenamepath)[1..],-55} {GetConciseDescription(uknBlock, new int[] { 8, 4, 7, 5 })}" +
                            $"{shaderFile.DBlocks[0].Name,-33} {shaderFile.DBlocks[1].Name}");
                    }

                    if (uknBlock.Rule == ConditionalRule.Allow && AllFlagsAre3(uknBlock) && uknBlock.ConditionalTypes.Length >= 2)
                    {
                    ...
                    if (uknBlock.Rule == ConditionalRule.Requires && uknBlock.ConditionalTypes.Length == 2 &&
                        uknBlock.ConditionalTypes[1] == ConditionalType.Static && uknBlock.Values.Length == 1)
                    {
                    ...
                    if (uknBlock.Rule == ConditionalRule.Requires && uknBlock.Values.Length == 0 && uknBlock.ConditionalTypes.Length >= 3)
                    {
                        Console.WriteLine($"{ShortHandName(vcsFilenamepath)[1..],-55} {GetConciseDescription(uknBlock, new int[] { 8, 4, 12, 5 })} " +
                            $"{GetResolvedNames(uknBlock, shaderFile.SfBlocks, shaderFile.DBlocks)}");
                    }
        ...
        public static string GetConciseDescription(ConstraintsBlock uknBlock, int[] usePadding = null)
        {
            int[] p = { 10, 8, 15, 5 };
            if (usePadding != null)
            {
        ...
        public static string GetResolvedNames(ConstraintsBlock dRuleBlock, List<SfBlock> sfBlocks, List<DBlock> dBlocks)
        {
            ...
            for (int i = 0; i < dRuleBlock.ConditionalTypes.Length; i++)
            {
                ...
                {
                    names.Add(sfBlocks[dRuleBlock.Indices[i]].Name);
                    continue;
                }
                if (dRuleBlock.ConditionalTypes[i] == ConditionalType.Dynamic)
                {
                    names.Add(dBlocks[dRuleBlock.Indices[i]].Name);
                    continue;
                }
        ...
        public static string RelRuleDescribe(ConstraintsBlock dRuleBlock)
        {
            return dRuleBlock.Rule == ConditionalRule.Allow ? "EXC(3)" : $"INC({dRuleBlock.Rule})";
        }
            ...
            foreach (string vcsFilenamepath in allVcsFiles)
            {
                ...
                foreach (ConstraintsBlock cBlock in shaderFile.SfConstraintsBlocks)
                {
                    if (cBlock.Range2[0] == 0)
                    {
                        ...
                        Console.WriteLine($"nr arguments {cBlock.Indices.Length} {shaderFile.SfBlocks[cBlock.Indices[0]].Name}");
                    }
                }
            }
        }
        ...
        static void CompatRuleKeyDescriptionSurvey()
        {
            ...
            foreach (string vcsFilenamepath in allVcsFiles)
            {
                ...
                foreach (ConstraintsBlock cBlock in shaderFile.SfConstraintsBlocks)
                {
                    string relRuleKeyDesciption = $"{RelRuleDescribe(cBlock),-10} {CombineValues2(cBlock.Values),-7} {CombineValues2(cBlock.Range2)}";
                    CollectStringValue(relRuleKeyDesciption);
                }
            }
        }
        ...
        static void ScanForValueSfBlocks()
        {
            ...
            foreach (string filenamepath in allVcsFiles)
            {
                ...
                foreach (SfBlock sfBlock in shaderFile.SfBlocks)
                {
                    ...
                    if (sfBlock.Sys == 1)
                    {
                        result.Add($"{sfBlock.Arg0,3} {sfBlock.RangeMax,3} {sfBlock.Sys,3} {sfBlock.FeatureIndex,3}     " +
        ...
        static void UnknownBlockConcise(string filenamepath, bool showLink = true)
        {
            ...
            foreach (ConstraintsBlock uBlock in shaderFile.DConstraintsBlocks)
            {
                if (newFile)
                {
                ...
                {
                    if (uBlock.ConditionalTypes[i] == ConditionalType.Dynamic)
                    {
                        uknNames[i] = shaderFile.DBlocks[uBlock.Indices[i]].Name;
                        continue;
                    }
                    if (uBlock.ConditionalTypes[i] == ConditionalType.Static)
                    {
                        uknNames[i] = shaderFile.SfBlocks[uBlock.Indices[i]].Name;
                        continue;
                    }
        ...
        static void ShowDBlockArgumentList(string filenamepath, bool showHtmlLink = true)
        {
            ...
            foreach (DBlock dBlock in shaderFile.DBlocks)
            {
                ...
                string v1 = dBlock.Name;
                string v2 = "" + dBlock.RangeMax;
                string v3 = "" + dBlock.Sys;
                string v4 = $"{dBlock.FeatureIndex,2}";
        ...
        static void ShowSfArgumentList(string filenamepath, bool showLink = true)
        {
            ...
            foreach (SfBlock sfBlock in shaderFile.SfBlocks)
            {
                ...
                string abbreviation = $"{sfBlock.Name}({ShortenShaderParam(sfBlock.Name).ToLower()})";
                abbreviationsUsed[abbreviation] = 1;

                string v2 = "" + sfBlock.RangeMax;
                string v3 = "" + sfBlock.Sys;
                ...
                string outputFilenamepath = null, bool writeFile = false, bool disableOutput = false)
        {
            ...
            foreach (SfBlock sfBlock in shaderFile.SfBlocks)
            {
                sfNames.Add(ShortenShaderParam(sfBlock.Name).ToLower());
            }
        ...
        static void CompatBlockDetailsConcise2(string filenamepath, bool showLink = true)
        {
            ...
            foreach (ConstraintsBlock cBlock in shaderFile.SfConstraintsBlocks)
            {
                ...
                newFile = false;
                string[] sfNames = new string[cBlock.Indices.Length];
                for (int i = 0; i < sfNames.Length; i++)
                {
                    sfNames[i] = shaderFile.SfBlocks[cBlock.Indices[i]].Name;
                }
                ...
                string s1 = (cBlock.Rule == ConditionalRule.Requires) ? $"INC({cBlock.Rule})" : $"EXC({cBlock.Rule})";
                // string s2 = $"{cBlock.arg0}";
                string s3 = $"{GetByteFlagsAsString(cBlock)}";
                // string s4 = $"{CombineValues(sfNames)}";
                string s4 = $"{breakNames[0]}";
                string s5 = $"{CombineValues2(cBlock.Indices)}";
                string s6 = $"{CombineValues2(cBlock.Values)}";
        ...
        static void CompatBlockRelationships()
        {
            ...
            foreach (string filenamepath in allVcsFiles)
            {
                ...
                foreach (ConstraintsBlock cBlock in shaderFile.SfConstraintsBlocks)
                {
                    ...
                    if (cBlock.BlockType == ConditionalType.Feature && shaderFile.VcsProgramType != VcsProgramType.Features)
                    {
                        Console.WriteLine($"error!");
                    }

                    if (cBlock.BlockType == ConditionalType.Static)
                    {
        ...
        static void CompatBlockDetailsConcise(string filenamepath)
        {
            ...
            {
                string s0 = $"[{cBlock.BlockIndex,2}]";
                string s1 = $"{cBlock.Rule}";
                string s2 = $"{cBlock.BlockType}";
                string s3 = $"{GetByteFlagsAsString(cBlock)}";
        ...
        static void CompatBlockDetailsSelectedFile(string filenamepath)
        {
            ...
            foreach (ConstraintsBlock cBlock in shaderFile.SfConstraintsBlocks)
            {
                ...
                OutputWriteLine($"rule = {cBlock.Rule}");
                OutputWriteLine($"arg1 = {cBlock.BlockType}");
                // OutputWriteLine($"{CombineValues(cBlock.flags)}");
                OutputWriteLine($"      {GetByteFlagsAsString(cBlock)}");
                OutputWriteLine($"{CombineValues(cBlock.Indices)}");
                OutputWriteLine($"{CombineValues(cBlock.Values)}");
        ...
        static void CompatibilityBlockValuesExample(string outputFilenamepath = null, bool writeFile = false)
        {
            ...
            ConstraintsBlock block0 = shaderFile.SfConstraintsBlocks[0];
